...
def reliabilityScoreTwitter(user):
    ...
    ratioFollowerFollowed = float(user["Followers"]) / float(Follows)
    ratioLikedTweet = float(user["Likes"]) / float(Tweets)
    ratioCommentTweet = float(user["Comment"]) / float(Tweets)
    ratioRetweetTweet = float(user["Retweet"]) / float(Tweets)
